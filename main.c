...
static unsigned int str_to_uint(const char *str, int *err)
{
        ...
        // We use the signed version of strtoll to avoid silent negation of
        // negative inputs (see strtoll(3)). This restricts our input to values
        // less than LLONG_MAX, which may or may not be less than UINT_MAX. In
        // practice, this means the user may be unable to set the most
        // significant bit of the seed on platforms where
